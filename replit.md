...
-   **Landing Page** (`/`): Full GTM-optimized landing page with: Hero section (14-day free trial badge, cost savings headline, dual CTAs), stats bar (2,400+ companies, 105+ roles, 99.9% uptime, 40-70% savings), 6 trust badges (SOC2 Type II, GDPR, ISO 27001, HIPAA Ready, 256-bit encryption, uptime SLA), problem section (3 pain-points with CTA), 8-feature grid, differentiation comparison table (vs Chatbots/Virtual Assistants/RPA/AI Copilots/Avatar Platforms with tabbed UI), 7 industry vertical content blocks (Healthcare/Financial Services/Technology/Legal/Retail/Manufacturing/Education with role examples and impact stats), how-it-works steps, demo video section (placeholder), testimonials, pricing section (4 plans with annual/monthly toggle, savings callouts up to $4,800/yr, trial messaging), interactive ROI calculator (role-based inputs with role selector dropdowns + headcount inputs, "add another role" functionality, plan selector, real-time savings calculation), email signup section with confirmation UI, FAQ accordion (8 items including trial and comparison questions), demo request form (email, company name, company size dropdown), CTA section with trial + login, 5-column footer.
